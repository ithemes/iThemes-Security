	...
	class BWPS_Content_Directory_Admin {
		...
		public function add_module_intro( $screen ) {
				...
				if ( $this->settings !== true ) {
					...
					$content .= sprintf( '<div class="bwps-warning-message"><span>%s: </span>%s</div>', __( 'WARNING', 'better-wp-security' ), __( 'Backup your WordPress installation before using this tool.', 'better-wp-security' ) );
					$content .= sprintf( '<div class="bwps-warning-message"><span>%s: </span>%s</div>', __( 'WARNING', 'better-wp-security' ), __( 'Renaming your <code>wp-content</code> directory will break links on a site with existing content.', 'better-wp-security' ) );
