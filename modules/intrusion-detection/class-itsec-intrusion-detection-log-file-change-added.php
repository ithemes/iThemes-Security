<?php
/**
 * Display Added Files Details for Intrusion Detection Module
 *
 * @package    iThemes-Security
 * @subpackage Intrusion-Detection
 * @since      4.0
 */
if ( ! class_exists( 'ITSEC_Intrusion_Detection_Log_File_Change_Added' ) ) {

	final class ITSEC_Intrusion_Detection_Log_File_Change_Added extends ITSEC_WP_List_Table {

		function __construct() {

			parent::__construct(
				array(
					'singular' => 'itsec_file_change_log_added_item',
					'plural'   => 'itsec_file_change_log_added_items',
					'ajax'     => true
				)
			);
		...
		function extra_tablenav( $which ) {
			...
			if ( $which == 'top' ) {
				echo '<h4>' . __( 'Files Added', 'ithemes-security' ) . '</h4>';
			}
